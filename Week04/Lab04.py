...
def robot_returns_to_origin(moves):
    ...
    for move in moves:
        if move == 'U':
            y += 1  # Move up increases y or y = y + 1
        elif move == 'D':
            y -= 1  # Move down decreases y or y = y - 1
        elif move == 'R':
            x += 1  # Move right increases x or x = x + 1
        elif move == 'L':
            x -= 1  # Move left decreases x or x = x - 1
    ...
    for i in range(len(numbers)):
        for j in range(i + 1, len(numbers)):
            if numbers[i] + numbers[j] == target:
                return (i, j)  # Return indices as a tuple
...
def two_sum_optimized(numbers, target):
    ...
    for i in range(len(numbers)):
        needed = target - numbers[i]  # The number we need to find
        if needed in seen:
            return (seen[needed], i)  # Return indices as a tuple
        seen[numbers[i]] = i  # Add current number and index to seen
    return None  # Return None if no pair found
...
def count_characters(s):
    ...
    for char in s:
        if char in counts:
            counts[char] += 1  # Increment count if already exists
        else:
            counts[char] = 1   # Initialize count to 1 if new character
    return counts
